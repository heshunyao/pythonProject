...
每种流量套餐产品包含三个属性：名称(name)，月费价格(price)，月流量(data04)。
...
有没有不限流量的：{"data04":{"operator":"==","value":"无上限"}}
流量大的：{"sort":{"ordering"="descend","value"="data04"}}
100G以上流量的套餐最便宜的是哪个：{"sort":{"ordering"="ascend","value"="price"},"data04":{"operator":">=","value":100}}
...
class MockedDB:
    def __init__(self):
        self.data = [
            {"name": "经济套餐", "price": 50, "data04": 10, "requirement": None},
            {"name": "畅游套餐", "price": 180, "data04": 100, "requirement": None},
            {"name": "无限套餐", "price": 300, "data04": 1000, "requirement": None},
            {"name": "校园套餐", "price": 150, "data04": 200, "requirement": "在校生"},
        ]
...
class DialogManager:
    def __init__(self, prompt_templates):
        ...
        self.session = [
            {
                "role": "system",
                "content": "你是一个手机流量套餐的客服代表，你叫小跃。可以帮助用户选择最合适的流量套餐产品。"
            }
        ]
        self.nlu = NLU()
        self.dst = DST()
...
prompt_templates = {
    ...
    "not_found": "用户说：__INPUT__ \n\n没有找到满足__PRICE__元价位，__DATA__G流量的产品，询问用户是否有其他选择倾向。"
}

#测试
dm = DialogManager(prompt_templates)
response = dm.run("300太贵了，10元有2G流量有吗")
print("===response===")
print(response)