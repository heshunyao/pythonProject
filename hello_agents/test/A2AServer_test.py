...
# 2. 启动所有服务
threading.Thread(target=lambda: researcher.run(port=8500), daemon=True).start()
threading.Thread(target=lambda: writer.run(port=8501), daemon=True).start()
threading.Thread(target=lambda: editor.run(port=8502), daemon=True).start()
time.sleep(2)  # 等待服务启动

# 3. 创建客户端连接到各个Agent
researcher_client = A2AClient("http://127.0.0.1:8500")
writer_client = A2AClient("http://127.0.0.1:8501")
editor_client = A2AClient("http://127.0.0.1:8502")
